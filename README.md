...
filepath <- 'C:/Users/Admin/OneDrive/Desktop/my code'
setwd(filepath)
...
test1 <- ur.df(mydata$lk,type="none",lags=0)
test1
...
model121 <- Arima(mydata$diff2lk,order=c(1,2,1),method="ML")
model121

#estimate ARIMA(1,2,0)
# (use functions in the forecast package)
model120 <- Arima(mydata$diff2lk,order=c(1,2,0),method="ML")
model120

#estimate ARIMA(0,2,1)
# (use functions in the forecast package)
model021 <- Arima(mydata$diff2lk,order=c(0,2,1),method="ML")
model021

#estimate ARIMA(2,2,1)
# (use functions in the forecast package)
model221 <- Arima(mydata$diff2lk,order=c(2,2,1),method="ML")
model221

#estimate ARIMA(1,2,2)
# (use functions in the forecast package)
model122 <- Arima(mydata$diff2lk,order=c(1,2,2),method="ML")
model122

# set end of sample for initial estimations
n.obs <- length(mydata$diff2lk)
n.end <- 36
...
# (use functions in the forecast package) 
accuracy(pred[,1],mydata$diff2lk[(n.end+1):n.obs])
accuracy(pred[,2],mydata$diff2lk[(n.end+1):n.obs])
...
layout(1)
plot(mydata$diff2lk,ann=FALSE, xlim=c(1980,2025))
